...
import (
	...
	"github.com/monkeydioude/donut/internal/consts"
...
func (g *Game) Update(gs *scene.GameState) error {
	...
	if gs.Keyboard.IsActionOn(consts.ZoomIn) {
		g.camera.AddZoom(0.01)
	}
	if gs.Keyboard.IsActionOn(consts.ZoomOut) {
		g.camera.AddZoom(-0.01)
	}
	if gs.Keyboard.IsActionOn(consts.Up) {
		g.camera.MovingY(10)
	}
	if gs.Keyboard.IsActionOn(consts.Down) {
		g.camera.MovingY(-10)
	}
	if gs.Keyboard.IsActionOn(consts.Left) {
		g.camera.MovingX(10)
	}
	if gs.Keyboard.IsActionOn(consts.Right) {
		g.camera.MovingX(-10)
	}

	if gs.Keyboard.IsActionOn(consts.ZoomReset) {
		g.camera.ResetZoom()
	}
	if gs.Keyboard.DidActionsJustReleased(consts.Up, consts.Down) {
		g.camera.MovingY(0)
	}
	if gs.Keyboard.DidActionsJustReleased(consts.Left, consts.Right) {
		g.camera.MovingX(0)
	}
	g.camera.Update()
	return nil
...
func (g Game) ComputeOffsets(screen *state.Screen, x, y int) (int, int) {
	...
	offsetY := 40
	return nScreenX + offsetX, nScreenY + offsetY
}
...
func (g Game) Draw(screen *ebiten.Image, gs *scene.GameState) {
	...
	for y := range GridRows {
		for x := range GridCols {
			iso := coords.NewIsometric(x, y, g.conf.TileWidth, g.conf.TileHeight)
			nw := gs.Screen.NormalizeXWithCamera(g.conf.TileWidth, g.conf.RefWidth, g.camera)
			nScreenX, nScreenY := gs.Screen.Normalize(iso.X, iso.Y, g.conf.RefWidth, g.conf.RefHeight)
			// Center the grid
			offsetX := gs.Screen.Width/2 - (GridCols*nw)/11
			offsetY := 40

			op := &ebiten.DrawImageOptions{}
			op.GeoM.Translate(float64(g.camera.CameraX(offsetX+nScreenX)), float64(g.camera.CameraY(offsetY+nScreenY)))
			if ((y*GridCols)+x)%2 == 0 {
